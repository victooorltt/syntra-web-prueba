...
Deno.serve(async (req) => {
  try {
    ...
    const emailFrom = Deno.env.get("EMAIL_FROM") || Deno.env.get("RESEND_FROM_EMAIL") || "onboarding@resend.dev";
    const emailTo = Deno.env.get("EMAIL_TO") || Deno.env.get("RESEND_TO_EMAIL") || "info@syntralabs.es";
...
Presupuesto indicado: ${lead.presupuesto || 'No especificado'}`
    ...
    // 3. Send Email Notification (Resilient: Resend with fallback to Base44 Native Email)
    ...
    if (resendApiKey && resendApiKey !== 'placeholder_resend_key') {
      try {
        ...
        console.error("Failed to send email via Resend, falling back to Base44 SendEmail:", resendError);
      }
    }

    // Fallback to Base44 Native Email Integration if Resend is missing or failed
    if (!emailSentViaResend) {
      try {
        await base44.asServiceRole.integrations.Core.SendEmail({
          to: emailTo,
          from_name: "Syntra Labs — Formulario Web",
          subject: `🔔 Nuevo lead: ${lead.nombre} — ${lead.nombre_negocio}`,
          body: `
<h2>Nuevo lead recibido desde el formulario web (Fallback)</h2>
<p>La notificación de Resend no pudo enviarse. Aquí están los detalles del lead:</p>

<table style="font-family: Arial, sans-serif; font-size: 14px; border-collapse: collapse;">
  <tr><td style="padding: 6px 12px; font-weight: bold; color: #555;">Nombre</td><td style="padding: 6px 12px;">${lead.nombre || '—'}</td></tr>
  <tr><td style="padding: 6px 12px; font-weight: bold; color: #555;">Email</td><td style="padding: 6px 12px;">${lead.email || '—'}</td></tr>
  <tr><td style="padding: 6px 12px; font-weight: bold; color: #555;">Teléfono</td><td style="padding: 6px 12px;">${lead.telefono || '—'}</td></tr>
  <tr><td style="padding: 6px 12px; font-weight: bold; color: #555;">Negocio</td><td style="padding: 6px 12px;">${lead.nombre_negocio || '—'}</td></tr>
  <tr><td style="padding: 6px 12px; font-weight: bold; color: #555;">Sector (IA)</td><td style="padding: 6px 12px;">${aiResult.sector}</td></tr>
  <tr><td style="padding: 6px 12px; font-weight: bold; color: #555;">Servicio (IA)</td><td style="padding: 6px 12px;">${aiResult.service}</td></tr>
  <tr><td style="padding: 6px 12px; font-weight: bold; color: #555;">Objetivos (IA)</td><td style="padding: 6px 12px;">${aiResult.goals}</td></tr>
  <tr><td style="padding: 6px 12px; font-weight: bold; color: #555;">Plazo (IA)</td><td style="padding: 6px 12px;">${aiResult.timeframe}</td></tr>
  <tr><td style="padding: 6px 12px; font-weight: bold; color: #555;">Presupuesto</td><td style="padding: 6px 12px;">${lead.presupuesto || 'No especificado'}</td></tr>
  <tr><td style="padding: 6px 12px; font-weight: bold; color: #555;">Mensaje original</td><td style="padding: 6px 12px;">${lead.mensaje || '—'}</td></tr>
  <tr><td style="padding: 6px 12px; font-weight: bold; color: #555;">Fecha</td><td style="padding: 6px 12px;">${new Date().toLocaleString('es-ES', { timeZone: 'Europe/Madrid' })}</td></tr>
</table>

<p style="margin-top: 20px; color: #888; font-size: 12px;">Este email ha sido generado automáticamente como método alternativo desde el formulario de contacto de syntralabs.es</p>
          `
        });
        console.log("Fallback notification email successfully sent via Base44 Core integration.");
      } catch (fallbackError) {
        console.error("Critical: Fallback email alert also failed:", fallbackError);
      }
    }
